---
layout: default
---
2026-06-14

今日无新增 OR 相关研究
## 📅 2026-06-13

今日无新增 OR 相关研究
## 📅 2026-06-12

今日无新增 OR 相关研究
## 📅 2026-06-11

今日无新增 OR 相关研究
## 📅 2026-06-10


### ReflectiChain：供应链世界模型中的认知基础
- **英文标题**: ReflectiChain: Epistemic Grounding in LLM-Driven World Models for Supply Chain Resilience
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Jia Luo
- **核心贡献**: 引入生成式供应链世界模型
- **实践价值**: 供应链抗风险能力
- **OR 技术关键词**: 大语言模型, 强化学习, 供应链世界模型
- **论文链接**: https://arxiv.org/abs/2606.10359v1
---
## 📅 2026-06-09


### 集成深度学习需求预测与多目标优化的循环咖啡供应链
- **英文标题**: Integrating Deep Learning Demand Forecasting with Multi-Objective Optimization for Circular Coffee Supply Chains: A Data-Driven Framework for Cost, Emissions, and Freshness Management
- **来源**: 📄 arXiv  ⭐ 10/10
- **作者**: Gerçek Budak, Faraz Gholamzadeh Gharehgheshlaghi, Melika Barjesteh Vaezi
- **核心贡献**: 深度学习预测需求
- **实践价值**: 优化咖啡供应链成本、排放和新鲜度管理
- **OR 技术关键词**: 深度学习, 需求预测, 多目标优化, 供应链管理
- **论文链接**: https://arxiv.org/abs/2606.08314v1
---

### 基于Transformer的供应链金融信用评分
- **英文标题**: TRUST-SCF: Transformer-based Risk Understanding and Scoring for Transactional Supply Chain Finance
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Mohammadamin Davoodabadi, Amirabbas Shakeri
- **核心贡献**: Transformer模型评估信用风险
- **实践价值**: 提升供应链金融信用评估精度
- **OR 技术关键词**: Transformer, 信用评分, 供应链金融
- **论文链接**: https://arxiv.org/abs/2606.08140v1
---

### 贪婪履约策略下的库存分配
- **英文标题**: Inventory Allocation Under the Greedy Fulfillment Policy: The (Potential) Perils of the Hindsight Approach
- **来源**: OR-Journal  ⭐ 8/10
- **核心贡献**: 研究贪婪履约政策的潜在问题
- **实践价值**: 库存分配优化
- **OR 技术关键词**: 贪婪履约, 库存分配, 供应链管理
- **论文链接**: https://doi.org/10.1287/opre.2024.0994
---
## 📅 2026-06-08

今日无新增 OR 相关研究
## 📅 2026-06-08


### 城市轨道交通需求预测中的分层预测协调
- **英文标题**: Hierarchical Forecast Reconciliation for Urban Rail Transit Demand Prediction under Operational Disruptions
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Dang Viet Anh Nguyen, Alma Fazlagic, Kristine Pryds Loft
- **核心贡献**: 提出分层预测协调方法
- **实践价值**: 应用于城市轨道交通需求预测
- **OR 技术关键词**: 城市轨道交通, 需求预测, 分层预测
- **论文链接**: https://arxiv.org/abs/2606.07044v1
---
## 📅 2026-06-07

今日无新增 OR 相关研究
## 📅 2026-06-06


### 学习补货：药品供应链 动态库存管理混合深度强化学习
- **英文标题**: Learning to replenish: A hybrid deep reinforcement learning for dynamic inventory management in the pharmaceutical supply chains
- **来源**: 📄 arXiv  ⭐ 10/10
- **作者**: Amandeep Kaur, Gyan Prakash
- **核心贡献**: 提出混合深度强化学习用于动态库存管理
- **实践价值**: 优化药品供应链库存，平衡库存和减少浪费
- **OR 技术关键词**: 强化学习, 库存管理, 药品供应链
- **论文链接**: https://arxiv.org/abs/2606.06201v1
---
## 📅 2026-06-05

今日无新增 OR 相关研究
## 📅 2026-06-04

今日无新增 OR 相关研究
## 📅 2026-06-03

今日无新增 OR 相关研究
## 📅 2026-06-02

今日无新增 OR 相关研究
## 📅 2026-06-01

今日无新增 OR 相关研究
## 📅 2026-06-01

今日无新增 OR 相关研究
## 📅 2026-05-31

今日无新增 OR 相关研究
## 📅 2026-05-30

今日无新增 OR 相关研究
## 📅 2026-05-29

今日无新增 OR 相关研究
## 📅 2026-05-28

今日无新增 OR 相关研究
## 📅 2026-05-27

今日无新增 OR 相关研究
## 📅 2026-05-26

今日无新增 OR 相关研究
## 📅 2026-05-25

今日无新增 OR 相关研究
## 📅 2026-05-24


### 深度强化学习在随机作业到达的柔性作业车间调度问题
- **英文标题**: Deep Reinforcement Learning for Flexible Job Shop Scheduling with Random Job Arrivals
- **来源**: 📄 arXiv  ⭐ 10/10
- **作者**: Yu Tang, Muhammad Zakwan, Efe Balta
- **核心贡献**: 提出了一种基于事件的深度强化学习方法
- **实践价值**: 工业制造业调度优化
- **OR 技术关键词**: 深度强化学习, 柔性作业车间调度, 随机作业到达
- **论文链接**: https://arxiv.org/abs/2605.22773v1
---

### 大规模语言模型在运筹学中的应用
- **英文标题**: Large Language Models for Operations Research: A Comprehensive Survey
- **来源**: 📄 arXiv  ⭐ 10/10
- **作者**: Xianchao Xiu, Jianhao Li, Jun Fan
- **核心贡献**: 综述了大规模语言模型在运筹学中的应用
- **实践价值**: 运筹学决策支持
- **OR 技术关键词**: 大规模语言模型, 运筹学
- **论文链接**: https://arxiv.org/abs/2605.20849v1
---

### 电动汽车充电系统的规划、调度和行为研究
- **英文标题**: Planning, Scheduling, and Behavior in EV Charging Systems: A Critical Survey and Trilemma Framework
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Peiyan Xiao, Yuheng Li, Ayan Mukhopadhyay
- **核心贡献**: 提出了一种三重困境框架
- **实践价值**: 电动汽车充电系统优化
- **OR 技术关键词**: 电动汽车充电系统, 规划, 调度
- **论文链接**: https://arxiv.org/abs/2605.21665v1
---

### COAgents：多代理框架用于学习和导航路由问题搜索空间
- **英文标题**: COAgents: Multi-Agent Framework to Learn and Navigate Routing Problems Search Space
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Oleksandr Yakovenko, Mahdi Mostajabdaveh, Cheikh Ahmed
- **核心贡献**: 提出了一种协作多代理框架
- **实践价值**: 车辆路由问题优化
- **OR 技术关键词**: 多代理, 路由问题, 搜索空间
- **论文链接**: https://arxiv.org/abs/2605.20618v1
---

### 一种迭代约束规划方法：将最大工作量约束集成到抢占式作业车间调度
- **英文标题**: An iterative Constraint Programming approach to integrate maximum workload constraints in preemptive jobshop scheduling
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Tanguy Terrien, Cyrille Briand
- **核心贡献**: 提出了一种迭代约束规划方法
- **实践价值**: 工业制造业调度优化
- **OR 技术关键词**: 迭代约束规划, 抢占式作业车间调度
- **论文链接**: https://arxiv.org/abs/2605.20387v1
---

### 预见式学习：通过多节点预测增强神经路由策略
- **英文标题**: Learning with Foresight: Enhancing Neural Routing Policy via Multi-Node Lookahead Prediction
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Xia Jiang, Yaoxin Wu, Yew-Soon Ong
- **核心贡献**: 提出多节点预测增强神经路由策略
- **实践价值**: 解决车辆路由问题
- **OR 技术关键词**: 神经路由, 多节点预测, 车辆路由
- **论文链接**: https://arxiv.org/abs/2605.19975v1
---

### 并行机器调度的启发式方法
- **英文标题**: Heuristic approaches for solving a bilevel optimistic scheduling problem on parallel machines
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Quentin Schau, Federico Della Croce, Olivier Ploton
- **核心贡献**: 提出解决并行机器调度问题的启发式方法
- **实践价值**: 解决并行机器调度问题
- **OR 技术关键词**: 并行机器调度, 启发式方法
- **论文链接**: https://arxiv.org/abs/2605.19760v1
---

### 潜在强化学习行动投影：通向图组合优化的普遍化和扩展
- **英文标题**: Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Franco Terranova, Guillermo Bernardez, Albert Cabellos-Aparicio
- **核心贡献**: 提出潜在强化学习行动投影方法
- **实践价值**: 解决图组合优化问题
- **OR 技术关键词**: 图组合优化, 潜在强化学习
- **论文链接**: https://arxiv.org/abs/2605.19721v1
---

### GOAL：基于图的目标对齐扩散求解器用于动态多目标优化
- **英文标题**: GOAL: Graph-based Objective-Aligned Diffusion Solvers for Dynamic Multi-Objective Optimization
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Xingyu Li
- **核心贡献**: 提出GOAL扩散求解器
- **实践价值**: 解决动态多目标优化问题
- **OR 技术关键词**: 图优化, 扩散求解器
- **论文链接**: https://arxiv.org/abs/2605.19119v1
---

### 可解释的程序化强化学习框架
- **英文标题**: Scheduling That Speaks: An Interpretable Programmatic Reinforcement Learning Framework
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Chengpeng Hu, Yingqian Zhang, Hendrik Baier
- **核心贡献**: 提出可解释的强化学习框架
- **实践价值**: 应用于调度优化
- **OR 技术关键词**: 强化学习, 调度优化, 可解释性
- **论文链接**: https://arxiv.org/abs/2605.18454v1
---

### 量子模型用于CVRPTW
- **英文标题**: Quantum Model for CVRPTW
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Imran Meghazi, Éric Bourreau
- **核心贡献**: 提出基于Grover Search的量子算法
- **实践价值**: 应用于车辆路径规划
- **OR 技术关键词**: 量子计算, 车辆路径规划, CVRPTW
- **论文链接**: https://arxiv.org/abs/2605.18393v1
---

### Benders分解方法用于k-防御性支配问题
- **英文标题**: A Benders Decomposition Approach for the k-Defensive Domination Problem
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Bilge Varol, Tınaz Ekim, Kübra Tanınmış
- **核心贡献**: 提出Benders分解方法
- **实践价值**: 应用于网络安全和紧急管理
- **OR 技术关键词**: Benders分解, 网络安全, 紧急管理
- **论文链接**: https://arxiv.org/abs/2605.17982v2
---

### Petri网诱导的启发式搜索
- **英文标题**: Petri Net Induced Heuristic Search for Resource Constrained Scheduling
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Ido Lublin, Dor Atzmon, Izack Cohen
- **核心贡献**: 提出Petri网诱导的启发式搜索算法
- **实践价值**: 应用于资源约束调度
- **OR 技术关键词**: Petri网, 启发式搜索, 资源约束调度
- **论文链接**: https://arxiv.org/abs/2605.15983v1
---

### 上下文学习用于数据驱动的审查库存控制
- **英文标题**: In-Context Learning for Data-Driven Censored Inventory Control
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Sohom Mukherjee, Anh-Duy Pham, Richard Pibernik
- **核心贡献**: 提出了一种上下文学习方法解决审查库存控制问题
- **实践价值**: 适用于库存管理和供应链优化
- **OR 技术关键词**: 库存控制, 审查, 上下文学习
- **论文链接**: https://arxiv.org/abs/2605.14840v1
---

### 通过神经网络可扩展地解决随机多路径旅行商问题
- **英文标题**: Scalable Solution of the Stochastic Multi-path Traveling Salesman Problem via Neural Networks
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Xiaochen Chou, Ludovica Di Marco, Enza Messina
- **核心贡献**: 提出了一种神经网络方法解决随机多路径旅行商问题
- **实践价值**: 适用于物流和交通管理
- **OR 技术关键词**: 旅行商问题, 神经网络, 随机优化
- **论文链接**: https://arxiv.org/abs/2605.14662v1
---

### 一种统一的基于强化学习的通用容量车辆路由问题框架
- **英文标题**: A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Wen Wang, Xiangchen Wu, Liang Wang
- **核心贡献**: 提出了一种统一的强化学习框架解决车辆路由问题
- **实践价值**: 适用于物流和交通管理
- **OR 技术关键词**: 车辆路由问题, 强化学习, 通用框架
- **论文链接**: https://arxiv.org/abs/2605.14416v1
---

### 将约束程序转换为局部搜索输入
- **英文标题**: Transforming Constraint Programs to Input for Local Search
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Jo Devriendt, Patrick De Causmaecker, Marc Denecker
- **核心贡献**: 提出将约束程序转换为局部搜索输入的方法
- **实践价值**: 解决组合优化问题
- **OR 技术关键词**: 约束程序, 局部搜索
- **论文链接**: https://arxiv.org/abs/2605.19671v1
---

### 民主化大规模重新优化：LLM引导模型修复
- **英文标题**: Democratizing Large-Scale Re-Optimization with LLM-Guided Model Patches
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Tinghan Ye, Arnaud Deza, Ved Mohan
- **核心贡献**: 提出LLM引导模型修复方法
- **实践价值**: 解决重新优化问题
- **OR 技术关键词**: 重新优化, LLM引导
- **论文链接**: https://arxiv.org/abs/2605.18692v1
---

### 超越推理时间搜索
- **英文标题**: Beyond Inference-Time Search: Reinforcement Learning Synthesizes Reusable Solvers
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Soheyl Massoudi, Gabriel Apaza, Milad Habibi
- **核心贡献**: 提出强化学习合成可复用求解器
- **实践价值**: 应用于组合优化
- **OR 技术关键词**: 强化学习, 组合优化, 求解器合成
- **论文链接**: https://arxiv.org/abs/2605.18374v1
---

### 记忆引导树搜索
- **英文标题**: Memory-Guided Tree Search with Cross-Branch Knowledge Transfer for LLM Solver Synthesis
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Fatemeh Haji, Javier Delarosa Quiros, Peyman Najafirad
- **核心贡献**: 提出记忆引导树搜索算法
- **实践价值**: 应用于组合优化
- **OR 技术关键词**: 树搜索, 组合优化, 记忆引导
- **论文链接**: https://arxiv.org/abs/2605.17539v2
---

### 混合LLM-based智能框架
- **英文标题**: Hybrid LLM-based Intelligent Framework for Robot Task Scheduling
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Swayamjit Saha, Subhabrata Das, Haonan Duan
- **核心贡献**: 提出混合LLM-based智能框架
- **实践价值**: 应用于机器人任务调度
- **OR 技术关键词**: LLM, 机器人任务调度, 智能框架
- **论文链接**: https://arxiv.org/abs/2605.15486v1
---

### 解决数据驱动的需求响应调度中的终端约束
- **英文标题**: Addressing Terminal Constraints in Data-Driven Demand Response Scheduling
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Maximilian Bloor, Martha White, Ehecatl Antonio del Rio Chanona
- **核心贡献**: 提出了一种方法解决电力市场中的终端约束问题
- **实践价值**: 适用于电力市场和能源管理
- **OR 技术关键词**: 需求响应, 终端约束, 调度
- **论文链接**: https://arxiv.org/abs/2605.14741v1
---

### PyCSP3-Scheduling：PyCSP3的调度扩展
- **英文标题**: PyCSP3-Scheduling: A Scheduling Extension for PyCSP3
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Sohaib Afifi
- **核心贡献**: 提出了一种调度扩展方法用于PyCSP3
- **实践价值**: 适用于调度和约束编程
- **OR 技术关键词**: PyCSP3, 调度, 约束编程
- **论文链接**: https://arxiv.org/abs/2605.14559v1
---

### 代理即时编译于基于Web的代理规划和调度
- **英文标题**: Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling
- **来源**: 📄 arXiv  ⭐ 7/10
- **作者**: Caleb Winston, Ron Yifeng Wang, Azalia Mirhoseini
- **核心贡献**: 提出了一种代理即时编译方法
- **实践价值**: 代理计划执行优化
- **OR 技术关键词**: 代理即时编译, Web代理, 规划和调度
- **论文链接**: https://arxiv.org/abs/2605.21470v1
---

### 比较神经和启发式求解器在组合优化中的耗能效率
- **英文标题**: An Amortized Efficiency Threshold for Comparing Neural and Heuristic Solvers in Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 7/10
- **作者**: Sohaib Afifi
- **核心贡献**: 提出了一种方法比较神经和启发式求解器的耗能效率
- **实践价值**: 适用于组合优化和人工智能
- **OR 技术关键词**: 组合优化, 神经网络, 耗能效率
- **论文链接**: https://arxiv.org/abs/2605.14624v2
---

### 在算法对齐下提炼保证用于组合优化
- **英文标题**: Towards Distillation Guarantees under Algorithmic Alignment for Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Thien Le, Melanie Weber
- **核心贡献**: 提出了算法对齐下的提炼保证
- **实践价值**: 模型压缩优化
- **OR 技术关键词**: 算法对齐, 提炼保证, 组合优化
- **论文链接**: https://arxiv.org/abs/2605.20074v1
---
## 📅 2026-05-24


### 迭代约束编程方法解决预抢占性任务调度问题
- **英文标题**: An iterative Constraint Programming approach to integrate maximum workload constraints in preemptive jobshop scheduling
- **来源**: 📄 arXiv  ⭐ 10/10
- **作者**: Tanguy Terrien, Cyrille Briand
- **核心贡献**: 提出迭代约束编程方法
- **实践价值**: 应用于预抢占性任务调度
- **OR 技术关键词**: 迭代约束编程, 任务调度, 预抢占
- **论文链接**: https://arxiv.org/abs/2605.20387v1
---

### 多智能体框架学习和导航路由问题搜索空间
- **英文标题**: COAgents: Multi-Agent Framework to Learn and Navigate Routing Problems Search Space
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Oleksandr Yakovenko, Mahdi Mostajabdaveh, Cheikh Ahmed
- **核心贡献**: 引入COAgents框架解决VRP
- **实践价值**: 应用于车辆路由问题
- **OR 技术关键词**: 多智能体, 路由问题, VRP
- **论文链接**: https://arxiv.org/abs/2605.20618v1
---

### 增强神经路由政策的多节点预测
- **英文标题**: Learning with Foresight: Enhancing Neural Routing Policy via Multi-Node Lookahead Prediction
- **来源**: 📄 arXiv  ⭐ 9/10
- **作者**: Xia Jiang, Yaoxin Wu, Yew-Soon Ong
- **核心贡献**: 提出多节点预测方法
- **实践价值**: 应用于车辆路由问题
- **OR 技术关键词**: 神经路由, 多节点预测, 车辆路由
- **论文链接**: https://arxiv.org/abs/2605.19975v1
---

### 层次优化平行机调度问题的启发式方法
- **英文标题**: Heuristic approaches for solving a bilevel optimistic scheduling problem on parallel machines
- **来源**: 📄 arXiv  ⭐ 8/10
- **作者**: Quentin Schau, Federico Della Croce, Olivier Ploton
- **核心贡献**: 提出启发式方法解决层次优化问题
- **实践价值**: 应用于平行机调度
- **OR 技术关键词**: 层次优化, 启发式方法, 平行机调度
- **论文链接**: https://arxiv.org/abs/2605.19760v1
---

### 组合优化下蒸馏保证的算法对齐
- **英文标题**: Towards Distillation Guarantees under Algorithmic Alignment for Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 7/10
- **作者**: Thien Le, Melanie Weber
- **核心贡献**: 提出组合优化下的蒸馏保证
- **实践价值**: 应用于结构预测
- **OR 技术关键词**: 组合优化, 蒸馏, 算法对齐
- **论文链接**: https://arxiv.org/abs/2605.20074v1
---
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-24


### Deep Reinforcement Learning for Flexible Job Shop Scheduling with Random Job Arrivals
- **英文标题**: Deep Reinforcement Learning for Flexible Job Shop Scheduling with Random Job Arrivals
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Yu Tang, Muhammad Zakwan, Efe Balta
- **论文链接**: https://arxiv.org/abs/2605.22773v1
---

### Planning, Scheduling, and Behavior in EV Charging Systems: A Critical Survey and Trilemma Framework
- **英文标题**: Planning, Scheduling, and Behavior in EV Charging Systems: A Critical Survey and Trilemma Framework
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Peiyan Xiao, Yuheng Li, Ayan Mukhopadhyay
- **论文链接**: https://arxiv.org/abs/2605.21665v1
---

### Dropout Universality: Scaling Laws and Optimal Scheduling at the Edge-of-Chaos
- **英文标题**: Dropout Universality: Scaling Laws and Optimal Scheduling at the Edge-of-Chaos
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Lucas Fernandez Sarmiento
- **论文链接**: https://arxiv.org/abs/2605.21648v1
---

### Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling
- **英文标题**: Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Caleb Winston, Ron Yifeng Wang, Azalia Mirhoseini
- **论文链接**: https://arxiv.org/abs/2605.21470v1
---

### Large Language Models for Operations Research: A Comprehensive Survey
- **英文标题**: Large Language Models for Operations Research: A Comprehensive Survey
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Xianchao Xiu, Jianhao Li, Jun Fan
- **论文链接**: https://arxiv.org/abs/2605.20849v1
---
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-24


### Planning, Scheduling, and Behavior in EV Charging Systems: A Critical Survey and Trilemma Framework
- **英文标题**: Planning, Scheduling, and Behavior in EV Charging Systems: A Critical Survey and Trilemma Framework
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Peiyan Xiao, Yuheng Li, Ayan Mukhopadhyay
- **论文链接**: https://arxiv.org/abs/2605.21665v1
---

### Dropout Universality: Scaling Laws and Optimal Scheduling at the Edge-of-Chaos
- **英文标题**: Dropout Universality: Scaling Laws and Optimal Scheduling at the Edge-of-Chaos
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Lucas Fernandez Sarmiento
- **论文链接**: https://arxiv.org/abs/2605.21648v1
---

### Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling
- **英文标题**: Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling
- **来源**: 📄 arXiv  ⭐ 6/10
- **作者**: Caleb Winston, Ron Yifeng Wang, Azalia Mirhoseini
- **论文链接**: https://arxiv.org/abs/2605.21470v1
---
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-24

今日无新增 OR 相关研究
## 📅 2026-05-23


### AutoRubric-T2I: Robust Rule-Based Reward Model for Text-to-Image Alignment
- **英文标题**: AutoRubric-T2I: Robust Rule-Based Reward Model for Text-to-Image Alignment
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Kuei-Chun Kao, Daixuan Huo, Yuanhao Ban
- **论文链接**: https://huggingface.co/papers/2605.17602
---

### Forecasting Scientific Progress with Artificial Intelligence
- **英文标题**: Forecasting Scientific Progress with Artificial Intelligence
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Sean Wu, Pan Lu, Yupeng Chen
- **论文链接**: https://huggingface.co/papers/2605.22681
---

### Efficient Agentic Reasoning Through Self-Regulated Simulative Planning
- **英文标题**: Efficient Agentic Reasoning Through Self-Regulated Simulative Planning
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Mingkai Deng, Jinyu Hou, Lara Sá Neves
- **论文链接**: https://huggingface.co/papers/2605.22138
---

### Forecasting Downstream Performance of LLMs With Proxy Metrics
- **英文标题**: Forecasting Downstream Performance of LLMs With Proxy Metrics
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Arkil Patel, Siva Reddy, Marius Mosbach
- **论文链接**: https://huggingface.co/papers/2605.18607
---

### Lean Refactor: Multi-Objective Controllable Proof Optimization via Agentic Strategy Search
- **英文标题**: Lean Refactor: Multi-Objective Controllable Proof Optimization via Agentic Strategy Search
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Jialin Lu, Soonho Kong, Rodrigo Stehling
- **论文链接**: https://huggingface.co/papers/2605.20244
---

### TransitLM: A Large-Scale Dataset and Benchmark for Map-Free Transit Route Generation
- **英文标题**: TransitLM: A Large-Scale Dataset and Benchmark for Map-Free Transit Route Generation
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Hanyu Guo, Jiedong Yang, Chao Chen
- **论文链接**: https://huggingface.co/papers/2605.22355
---

### Full Attention Strikes Back: Transferring Full Attention into Sparse within Hundred Training Steps
- **英文标题**: Full Attention Strikes Back: Transferring Full Attention into Sparse within Hundred Training Steps
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Yanke Zhou, Yiduo Li, Hanlin Tang
- **论文链接**: https://huggingface.co/papers/2605.16928
---

### One Sentence, One Drama: Personalized Short-Form Drama Generation via Multi-Agent Systems
- **英文标题**: One Sentence, One Drama: Personalized Short-Form Drama Generation via Multi-Agent Systems
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Yufei Shi, Weilong Yan, Naixuan Huang
- **论文链接**: https://huggingface.co/papers/2605.22144
---

### DelTA: Discriminative Token Credit Assignment for Reinforcement Learning from Verifiable Rewards
- **英文标题**: DelTA: Discriminative Token Credit Assignment for Reinforcement Learning from Verifiable Rewards
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Kaiyi Zhang, Wei Wu, Yankai Lin
- **论文链接**: https://huggingface.co/papers/2605.21467
---

### KVServe: Service-Aware KV Cache Compression for Communication-Efficient Disaggregated LLM Serving
- **英文标题**: KVServe: Service-Aware KV Cache Compression for Communication-Efficient Disaggregated LLM Serving
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Zedong Liu, Xinyang Ma, Dejun Luo
- **论文链接**: https://huggingface.co/papers/2605.13734
---

### From Reasoning Chains to Verifiable Subproblems: Curriculum Reinforcement Learning Enables Credit Assignment for LLM Reasoning
- **英文标题**: From Reasoning Chains to Verifiable Subproblems: Curriculum Reinforcement Learning Enables Credit Assignment for LLM Reasoning
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Xitai Jiang, Zihan Tang, Wenze Lin
- **论文链接**: https://huggingface.co/papers/2605.22074
---

### Bernini: Latent Semantic Planning for Video Diffusion
- **英文标题**: Bernini: Latent Semantic Planning for Video Diffusion
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Bernini Team, Chenchen Liu, Junyi Chen
- **论文链接**: https://huggingface.co/papers/2605.22344
---

### Spreadsheet-RL: Advancing Large Language Model Agents on Realistic Spreadsheet Tasks via Reinforcement Learning
- **英文标题**: Spreadsheet-RL: Advancing Large Language Model Agents on Realistic Spreadsheet Tasks via Reinforcement Learning
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Banghao Chi, Yining Xie, Mingyuan Wu
- **论文链接**: https://huggingface.co/papers/2605.22642
---
## 📅 2026-05-22


### ACC: Compiling Agent Trajectories for Long-Context Training
- **英文标题**: ACC: Compiling Agent Trajectories for Long-Context Training
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Qisheng Su, Zhen Fang, Shiting Huang
- **论文链接**: https://huggingface.co/papers/2605.21850
---

### TIDE: Efficient and Lossless MoE Diffusion LLM Inference with I/O-aware Expert Offload
- **英文标题**: TIDE: Efficient and Lossless MoE Diffusion LLM Inference with I/O-aware Expert Offload
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Zhiben Chen, Youpeng Zhao, Yang Sui
- **论文链接**: https://huggingface.co/papers/2605.20179
---

### Toto 2.0: Time Series Forecasting Enters the Scaling Era
- **英文标题**: Toto 2.0: Time Series Forecasting Enters the Scaling Era
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Emaad Khwaja, Chris Lettieri, Gerald Woo
- **论文链接**: https://huggingface.co/papers/2605.20119
---

### Mega-ASR: Towards In-the-wild^2 Speech Recognition via Scaling up Real-world Acoustic Simulation
- **英文标题**: Mega-ASR: Towards In-the-wild^2 Speech Recognition via Scaling up Real-world Acoustic Simulation
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Zhifei Xie, Kaiyu Pang, Haobin Zhang
- **论文链接**: https://huggingface.co/papers/2605.19833
---

### MOCHA: Multi-Objective Chebyshev Annealing for Agent Skill Optimization
- **英文标题**: MOCHA: Multi-Objective Chebyshev Annealing for Agent Skill Optimization
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Md Mehrab Tanjim, Jayakumar Subramanian, Xiang Chen
- **论文链接**: https://huggingface.co/papers/2605.19330
---

### CutVerse: A Compositional GUI Agents Benchmark for Media Post-Production Editing
- **英文标题**: CutVerse: A Compositional GUI Agents Benchmark for Media Post-Production Editing
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Haobo Hu, Xiangwu Guo, Zhiheng Chen
- **论文链接**: https://huggingface.co/papers/2605.19484
---

### Mix-Quant: Quantized Prefilling, Precise Decoding for Agentic LLMs
- **英文标题**: Mix-Quant: Quantized Prefilling, Precise Decoding for Agentic LLMs
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Haiquan Lu, Zigeng Chen, Gongfan Fang
- **论文链接**: https://huggingface.co/papers/2605.20315
---

### The Unlearnability Phenomenon in RLVR for Language Models
- **英文标题**: The Unlearnability Phenomenon in RLVR for Language Models
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Yulin Chen, He He, Chen Zhao
- **论文链接**: https://huggingface.co/papers/2605.16787
---

### Conditional Equivalence of DPO and RLHF: Implicit Assumption, Failure Modes, and Provable Alignment
- **英文标题**: Conditional Equivalence of DPO and RLHF: Implicit Assumption, Failure Modes, and Provable Alignment
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Zhiqin Yang, Yonggang Zhang, Wei Xue
- **论文链接**: https://huggingface.co/papers/2605.20834
---

### HRM-Text: Efficient Pretraining Beyond Scaling
- **英文标题**: HRM-Text: Efficient Pretraining Beyond Scaling
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Guan Wang, Changling Liu, Chenyu Wang
- **论文链接**: https://huggingface.co/papers/2605.20613
---

### PlanningBench: Generating Scalable and Verifiable Planning Data for Evaluating and Training Large Language Models
- **英文标题**: PlanningBench: Generating Scalable and Verifiable Planning Data for Evaluating and Training Large Language Models
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Ziliang Zhao, Zenan Xu, Shuting Wang
- **论文链接**: https://huggingface.co/papers/2605.20873
---

### Evaluating Temporal Semantic Caching and Workflow Optimization in Agentic Plan-Execute Pipelines
- **英文标题**: Evaluating Temporal Semantic Caching and Workflow Optimization in Agentic Plan-Execute Pipelines
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Alimurtaza Mustafa Merchant, Krish Veera, Sajal Kumar Goyla
- **论文链接**: https://huggingface.co/papers/2605.20630
---
## 📅 2026-05-21


### SENSE: Satellite-based ENergy Synthesis for Sustainable Environment
- **英文标题**: SENSE: Satellite-based ENergy Synthesis for Sustainable Environment
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Kailai Sun, Mingyi He, Heye Huang
- **论文链接**: https://huggingface.co/papers/2605.18101
---

### Not Every Rubric Teaches Equally: Policy-Aware Rubric Rewards for RLVR
- **英文标题**: Not Every Rubric Teaches Equally: Policy-Aware Rubric Rewards for RLVR
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Utkarsh Tyagi, Xingang Guo, MohammadHossein Rezaei
- **论文链接**: https://huggingface.co/papers/2605.20164
---

### Bug or Feature^2: Weight Drift, Activation Sparsity, and Spikes
- **英文标题**: Bug or Feature^2: Weight Drift, Activation Sparsity, and Spikes
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Egor Shvetsov, Aleksandr Serkov, Shokorov Viacheslav
- **论文链接**: https://huggingface.co/papers/2605.17659
---

### optimize_anything: A Universal API for Optimizing any Text Parameter
- **英文标题**: optimize_anything: A Universal API for Optimizing any Text Parameter
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Lakshya A Agrawal, Donghyun Lee, Shangyin Tan
- **论文链接**: https://huggingface.co/papers/2605.19633
---

### Overcoming Catastrophic Forgetting in Visual Continual Learning with Reinforcement Fine-Tuning
- **英文标题**: Overcoming Catastrophic Forgetting in Visual Continual Learning with Reinforcement Fine-Tuning
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Meng Lou, Hanzhong Guo, Linwei Chen
- **论文链接**: https://huggingface.co/papers/2605.09640
---

### TideGS: Scalable Training of Over One Billion 3D Gaussian Splatting Primitives via Out-of-Core Optimization
- **英文标题**: TideGS: Scalable Training of Over One Billion 3D Gaussian Splatting Primitives via Out-of-Core Optimization
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Chonghao Zhong, Linfeng Shi, Hua Chen
- **论文链接**: https://huggingface.co/papers/2605.20150
---

### SAGA: A Sequence-Adaptive Generative Architecture for Multi-Horizon Probabilistic Forecasting with Adaptive Temporal Conformal Prediction
- **英文标题**: SAGA: A Sequence-Adaptive Generative Architecture for Multi-Horizon Probabilistic Forecasting with Adaptive Temporal Conformal Prediction
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Gustav Olaf Yunus Laitinen-Fredriksson Lundström-Imanov, Hafize Gonca Cömert
- **论文链接**: https://huggingface.co/papers/2605.19014
---

### Video Models Can Reason with Verifiable Rewards
- **英文标题**: Video Models Can Reason with Verifiable Rewards
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Tinghui Zhu, Sheng Zhang, James Y. Huang
- **论文链接**: https://huggingface.co/papers/2605.15458
---

### CogOmniControl: Reasoning-Driven Controllable Video Generation via Creative Intent Cognition
- **英文标题**: CogOmniControl: Reasoning-Driven Controllable Video Generation via Creative Intent Cognition
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Hongji Yang, Songlian Li, Yucheng Zhou
- **论文链接**: https://huggingface.co/papers/2605.19995
---

### Aurora: Unified Video Editing with a Tool-Using Agent
- **英文标题**: Aurora: Unified Video Editing with a Tool-Using Agent
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Yongsheng Yu, Ziyun Zeng, Zhiyuan Xiao
- **论文链接**: https://huggingface.co/papers/2605.18748
---

### Delta Attention Residuals
- **英文标题**: Delta Attention Residuals
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Cheng Luo, Zefan Cai, Junjie Hu
- **论文链接**: https://huggingface.co/papers/2605.18855
---

### Echo-Forcing: A Scene Memory Framework for Interactive Long Video Generation
- **英文标题**: Echo-Forcing: A Scene Memory Framework for Interactive Long Video Generation
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Mingqiang Wu, Weilun Feng, Zhefeng Zhang
- **论文链接**: https://huggingface.co/papers/2605.16003
---

### DocAtlas: Multilingual Document Understanding Across 80+ Languages
- **英文标题**: DocAtlas: Multilingual Document Understanding Across 80+ Languages
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Ahmed Heakl, Youssef Mohamed, Abdullah Sohail
- **论文链接**: https://huggingface.co/papers/2605.12623
---

### Large Language Models for Operations Research: A Comprehensive Survey
- **英文标题**: Large Language Models for Operations Research: A Comprehensive Survey
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Xianchao Xiu, Jianhao Li, Jun Fan
- **论文链接**: https://arxiv.org/abs/2605.20849v1
---

### COAgents: Multi-Agent Framework to Learn and Navigate Routing Problems Search Space
- **英文标题**: COAgents: Multi-Agent Framework to Learn and Navigate Routing Problems Search Space
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Oleksandr Yakovenko, Mahdi Mostajabdaveh, Cheikh Ahmed
- **论文链接**: https://arxiv.org/abs/2605.20618v1
---

### An iterative Constraint Programming approach to integrate maximum workload constraints in preemptive jobshop scheduling
- **英文标题**: An iterative Constraint Programming approach to integrate maximum workload constraints in preemptive jobshop scheduling
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Tanguy Terrien, Cyrille Briand
- **论文链接**: https://arxiv.org/abs/2605.20387v1
---

### Towards Distillation Guarantees under Algorithmic Alignment for Combinatorial Optimization
- **英文标题**: Towards Distillation Guarantees under Algorithmic Alignment for Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Thien Le, Melanie Weber
- **论文链接**: https://arxiv.org/abs/2605.20074v1
---

### Learning with Foresight: Enhancing Neural Routing Policy via Multi-Node Lookahead Prediction
- **英文标题**: Learning with Foresight: Enhancing Neural Routing Policy via Multi-Node Lookahead Prediction
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Xia Jiang, Yaoxin Wu, Yew-Soon Ong
- **论文链接**: https://arxiv.org/abs/2605.19975v1
---

### Heuristic approaches for solving a bilevel optimistic scheduling problem on parallel machines
- **英文标题**: Heuristic approaches for solving a bilevel optimistic scheduling problem on parallel machines
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Quentin Schau, Federico Della Croce, Olivier Ploton
- **论文链接**: https://arxiv.org/abs/2605.19760v1
---

### Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization
- **英文标题**: Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Franco Terranova, Guillermo Bernardez, Albert Cabellos-Aparicio
- **论文链接**: https://arxiv.org/abs/2605.19721v1
---

### Transforming Constraint Programs to Input for Local Search
- **英文标题**: Transforming Constraint Programs to Input for Local Search
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Jo Devriendt, Patrick De Causmaecker, Marc Denecker
- **论文链接**: https://arxiv.org/abs/2605.19671v1
---

### Towards Multi-Model LLM Schedulers: Empirical Insights into Offloading and Preemption
- **英文标题**: Towards Multi-Model LLM Schedulers: Empirical Insights into Offloading and Preemption
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Mert Yildiz, Pietro Spadaccino, Alexey Rolich
- **论文链接**: https://arxiv.org/abs/2605.19593v1
---

### Beyond Mode Collapse: Distribution Matching for Diverse Reasoning
- **英文标题**: Beyond Mode Collapse: Distribution Matching for Diverse Reasoning
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Xiaozhe Li, Yang Li, Xinyu Fang
- **论文链接**: https://arxiv.org/abs/2605.19461v1
---

### Distributed Gradient-Regularized Newton Method: Scheduled Consensus and O(epsilon^{-1}) Global Iteration Complexity
- **英文标题**: Distributed Gradient-Regularized Newton Method: Scheduled Consensus and O(epsilon^{-1}) Global Iteration Complexity
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Wei Hu, Pengcheng Xie, Ya-Xiang Yuan
- **论文链接**: https://arxiv.org/abs/2605.19396v1
---

### GOAL: Graph-based Objective-Aligned Diffusion Solvers for Dynamic Multi-Objective Optimization
- **英文标题**: GOAL: Graph-based Objective-Aligned Diffusion Solvers for Dynamic Multi-Objective Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Xingyu Li
- **论文链接**: https://arxiv.org/abs/2605.19119v1
---

### Learning Interpretable Point-Based Clinical Risk Scores via Direct Optimization
- **英文标题**: Learning Interpretable Point-Based Clinical Risk Scores via Direct Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Ying Cui, Albert M Li, Vivek Charu
- **论文链接**: https://arxiv.org/abs/2605.19113v1
---

### Heterogeneity-Aware Dataset Scheduling for Efficient Audio Large Language Model Training
- **英文标题**: Heterogeneity-Aware Dataset Scheduling for Efficient Audio Large Language Model Training
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Yanru Wu, Jianning Wang, Chongxin Gan
- **论文链接**: https://arxiv.org/abs/2605.19101v1
---

### Democratizing Large-Scale Re-Optimization with LLM-Guided Model Patches
- **英文标题**: Democratizing Large-Scale Re-Optimization with LLM-Guided Model Patches
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Tinghan Ye, Arnaud Deza, Ved Mohan
- **论文链接**: https://arxiv.org/abs/2605.18692v1
---

### GAMMA: Global Bit Allocation for Mixed-Precision Models under Arbitrary Budgets
- **英文标题**: GAMMA: Global Bit Allocation for Mixed-Precision Models under Arbitrary Budgets
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Zhangyang Yao, Haiyan Zhao, Haoyu Wang
- **论文链接**: https://arxiv.org/abs/2605.18475v1
---

### Scheduling That Speaks: An Interpretable Programmatic Reinforcement Learning Framework
- **英文标题**: Scheduling That Speaks: An Interpretable Programmatic Reinforcement Learning Framework
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Chengpeng Hu, Yingqian Zhang, Hendrik Baier
- **论文链接**: https://arxiv.org/abs/2605.18454v1
---

### Quantum Model for CVRPTW
- **英文标题**: Quantum Model for CVRPTW
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Imran Meghazi, Éric Bourreau
- **论文链接**: https://arxiv.org/abs/2605.18393v1
---

### Beyond Inference-Time Search: Reinforcement Learning Synthesizes Reusable Solvers
- **英文标题**: Beyond Inference-Time Search: Reinforcement Learning Synthesizes Reusable Solvers
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Soheyl Massoudi, Gabriel Apaza, Milad Habibi
- **论文链接**: https://arxiv.org/abs/2605.18374v1
---

### Scalable Decision-Focused Learning through Cost-Sensitive Regression
- **英文标题**: Scalable Decision-Focused Learning through Cost-Sensitive Regression
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Noah Schutte, Senne Berden, Tias Guns
- **论文链接**: https://arxiv.org/abs/2605.18005v1
---

### A Benders Decomposition Approach for the k-Defensive Domination Problem
- **英文标题**: A Benders Decomposition Approach for the k-Defensive Domination Problem
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Bilge Varol, Tınaz Ekim, Kübra Tanınmış
- **论文链接**: https://arxiv.org/abs/2605.17982v2
---

### Bayesian-Monte Carlo Schedule Updating for Construction Digital Twins: A Probabilistic Framework for Dynamic Project Forecasting
- **英文标题**: Bayesian-Monte Carlo Schedule Updating for Construction Digital Twins: A Probabilistic Framework for Dynamic Project Forecasting
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Atena Khoshkonesh, Mohsen Mohammadagha, Vinayak Kaushal
- **论文链接**: https://arxiv.org/abs/2605.17608v1
---

### Memory-Guided Tree Search with Cross-Branch Knowledge Transfer for LLM Solver Synthesis
- **英文标题**: Memory-Guided Tree Search with Cross-Branch Knowledge Transfer for LLM Solver Synthesis
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Fatemeh Haji, Javier Delarosa Quiros, Peyman Najafirad
- **论文链接**: https://arxiv.org/abs/2605.17539v2
---

### Noise scheduling and linear dynamics in diffusion models on Lie groups
- **英文标题**: Noise scheduling and linear dynamics in diffusion models on Lie groups
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Javad Komijani
- **论文链接**: https://arxiv.org/abs/2605.17326v1
---

### Latent Heuristic Search: Continuous Optimization for Automated Algorithm Design
- **英文标题**: Latent Heuristic Search: Continuous Optimization for Automated Algorithm Design
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Cheikh Ahmed, Mahdi Mostajabdaveh, Zirui Zhou
- **论文链接**: https://arxiv.org/abs/2605.17137v1
---

### Learning Multi-Timescale Abstractions for Hierarchical Combinatorial Planning
- **英文标题**: Learning Multi-Timescale Abstractions for Hierarchical Combinatorial Planning
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Vivienne Huiling Wang, Tinghuai Wang, Joni Pajarinen
- **论文链接**: https://arxiv.org/abs/2605.17058v1
---

### Reliability and Effectiveness of Autonomous AI Agents in Supply Chain Management
- **英文标题**: Reliability and Effectiveness of Autonomous AI Agents in Supply Chain Management
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Carol Xuan Long, David Simchi-Levi, Feng Zhu
- **论文链接**: https://arxiv.org/abs/2605.17036v1
---

### Petri Net Induced Heuristic Search for Resource Constrained Scheduling
- **英文标题**: Petri Net Induced Heuristic Search for Resource Constrained Scheduling
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Ido Lublin, Dor Atzmon, Izack Cohen
- **论文链接**: https://arxiv.org/abs/2605.15983v1
---

### Optimizing Line Segment Inspection with Limited-Range Drones
- **英文标题**: Optimizing Line Segment Inspection with Limited-Range Drones
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: José-Miguel Díaz-Báñez, José-Manuel Higes, Alina Kasiuk
- **论文链接**: https://arxiv.org/abs/2605.15765v1
---

### Hybrid LLM-based Intelligent Framework for Robot Task Scheduling
- **英文标题**: Hybrid LLM-based Intelligent Framework for Robot Task Scheduling
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Swayamjit Saha, Subhabrata Das, Haonan Duan
- **论文链接**: https://arxiv.org/abs/2605.15486v1
---

### In-Context Learning for Data-Driven Censored Inventory Control
- **英文标题**: In-Context Learning for Data-Driven Censored Inventory Control
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Sohom Mukherjee, Anh-Duy Pham, Richard Pibernik
- **论文链接**: https://arxiv.org/abs/2605.14840v1
---

### Beyond What to Select: A Plug-and-play Oscillatory Data-Volume Scheduling for Efficient Model Training
- **英文标题**: Beyond What to Select: A Plug-and-play Oscillatory Data-Volume Scheduling for Efficient Model Training
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Suorong Yang, Hanqi Zhu, Hai Gan
- **论文链接**: https://arxiv.org/abs/2605.14773v1
---

### Addressing Terminal Constraints in Data-Driven Demand Response Scheduling
- **英文标题**: Addressing Terminal Constraints in Data-Driven Demand Response Scheduling
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Maximilian Bloor, Martha White, Ehecatl Antonio del Rio Chanona
- **论文链接**: https://arxiv.org/abs/2605.14741v1
---

### Scalable Solution of the Stochastic Multi-path Traveling Salesman Problem via Neural Networks
- **英文标题**: Scalable Solution of the Stochastic Multi-path Traveling Salesman Problem via Neural Networks
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Xiaochen Chou, Ludovica Di Marco, Enza Messina
- **论文链接**: https://arxiv.org/abs/2605.14662v1
---

### An Amortized Efficiency Threshold for Comparing Neural and Heuristic Solvers in Combinatorial Optimization
- **英文标题**: An Amortized Efficiency Threshold for Comparing Neural and Heuristic Solvers in Combinatorial Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Sohaib Afifi
- **论文链接**: https://arxiv.org/abs/2605.14624v2
---

### Uncovering latent consensus in heterogeneous populations: The Mixture Linear Ordering Problem
- **英文标题**: Uncovering latent consensus in heterogeneous populations: The Mixture Linear Ordering Problem
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Juan A. Aledo, Concepción Domínguez, Juan de Dios Jaime-Alcántara
- **论文链接**: https://arxiv.org/abs/2605.14596v1
---

### PyCSP3-Scheduling: A Scheduling Extension for PyCSP3
- **英文标题**: PyCSP3-Scheduling: A Scheduling Extension for PyCSP3
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Sohaib Afifi
- **论文链接**: https://arxiv.org/abs/2605.14559v1
---

### OPTIMUS: Optimization Productivity Tool for Intelligent Management of Utilizable Space
- **英文标题**: OPTIMUS: Optimization Productivity Tool for Intelligent Management of Utilizable Space
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Souvik Bhattacharyya, Nisha Singh, Salman Haider
- **论文链接**: https://arxiv.org/abs/2605.14430v1
---

### A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems
- **英文标题**: A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Wen Wang, Xiangchen Wu, Liang Wang
- **论文链接**: https://arxiv.org/abs/2605.14416v1
---

### Optimal Pattern Detection Tree for Symbolic Rule-Based Classification
- **英文标题**: Optimal Pattern Detection Tree for Symbolic Rule-Based Classification
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Young-Chae Hong, Yangho Chen
- **论文链接**: https://arxiv.org/abs/2605.14374v1
---

### Distribution-Aware Algorithm Design with LLM Agents
- **英文标题**: Distribution-Aware Algorithm Design with LLM Agents
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Saharsh Koganti, Priyadarsi Mishra, Pierfrancesco Beneventano
- **论文链接**: https://arxiv.org/abs/2605.14141v1
---

### Regret Equals Covariance: A Closed-Form Characterization for Stochastic Optimization
- **英文标题**: Regret Equals Covariance: A Closed-Form Characterization for Stochastic Optimization
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Irene Aldridge
- **论文链接**: https://arxiv.org/abs/2605.14019v1
---

### Graph Neural Networks with Triangle-Based Messages for the Multicut Problem
- **英文标题**: Graph Neural Networks with Triangle-Based Messages for the Multicut Problem
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Jannik Irmai, Lucas Fabian Naumann, Bjoern Andres
- **论文链接**: https://arxiv.org/abs/2605.13673v1
---

### CO-MAP: A Reinforcement Learning Approach to the Qubit Allocation Problem
- **英文标题**: CO-MAP: A Reinforcement Learning Approach to the Qubit Allocation Problem
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Ankit Kulshrestha, Xiaoyuan Liu
- **论文链接**: https://arxiv.org/abs/2605.13638v1
---

### An Agentic AI Framework with Large Language Models and Chain-of-Thought for UAV-Assisted Logistics Scheduling with Mobile Edge Computing
- **英文标题**: An Agentic AI Framework with Large Language Models and Chain-of-Thought for UAV-Assisted Logistics Scheduling with Mobile Edge Computing
- **来源**: 📄 arXiv  ⭐ 5/10
- **作者**: Hanwen Zhang, Dusit Niyato, Wei Zhang
- **论文链接**: https://arxiv.org/abs/2605.13221v1
---
## 📅 2026-05-20


### WavFlow: Audio Generation in Waveform Space
- **英文标题**: WavFlow: Audio Generation in Waveform Space
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Feiyan Zhou, Luyuan Wang, Shoufa Chen
- **论文链接**: https://huggingface.co/papers/2605.18749
---

### Symmetry-Compatible Principle for Optimizer Design: Embeddings, LM Heads, SwiGLU MLPs, and MoE Routers
- **英文标题**: Symmetry-Compatible Principle for Optimizer Design: Embeddings, LM Heads, SwiGLU MLPs, and MoE Routers
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Tim Tsz-Kit Lau, Weijie Su
- **论文链接**: https://huggingface.co/papers/2605.18106
---

### DexHoldem: Playing Texas Hold'em with Dexterous Embodied System
- **英文标题**: DexHoldem: Playing Texas Hold'em with Dexterous Embodied System
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Feng Chen, Tianzhe Chu, Li Sun
- **论文链接**: https://huggingface.co/papers/2605.18727
---

### TopoPrimer: The Missing Topological Context in Forecasting Models
- **英文标题**: TopoPrimer: The Missing Topological Context in Forecasting Models
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Zara Zetlin, Kayhan Moharreri, Maria Safi
- **论文链接**: https://huggingface.co/papers/2605.15035
---

### SafeDiffusion-R1: Online Reward Steering for Safe Diffusion Post-Training
- **英文标题**: SafeDiffusion-R1: Online Reward Steering for Safe Diffusion Post-Training
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Komal Kumar, Ankan Deria, Abhishek Basu
- **论文链接**: https://huggingface.co/papers/2605.18719
---

### AgentKernelArena: Generalization-Aware Benchmarking of GPU Kernel Optimization Agents
- **英文标题**: AgentKernelArena: Generalization-Aware Benchmarking of GPU Kernel Optimization Agents
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Sharareh Younesian, Wenwen Ouyang, Sina Rafati
- **论文链接**: https://huggingface.co/papers/2605.16819
---

### AtlasVA: Self-Evolving Visual Skill Memory for Teacher-Free VLM Agents
- **英文标题**: AtlasVA: Self-Evolving Visual Skill Memory for Teacher-Free VLM Agents
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Pan Wang, Yihao Hu, Xiujin Liu
- **论文链接**: https://huggingface.co/papers/2605.17933
---

### Code as Agent Harness
- **英文标题**: Code as Agent Harness
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Xuying Ning, Katherine Tieu, Dongqi Fu
- **论文链接**: https://huggingface.co/papers/2605.18747
---

### Lance: Unified Multimodal Modeling by Multi-Task Synergy
- **英文标题**: Lance: Unified Multimodal Modeling by Multi-Task Synergy
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Fengyi Fu, Mengqi Huang, Shaojin Wu
- **论文链接**: https://huggingface.co/papers/2605.18678
---

### KVPO: ODE-Native GRPO for Autoregressive Video Alignment via KV Semantic Exploration
- **英文标题**: KVPO: ODE-Native GRPO for Autoregressive Video Alignment via KV Semantic Exploration
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Ruicheng Zhang, Kaixi Cong, Jun Zhou
- **论文链接**: https://huggingface.co/papers/2605.14278
---
## 📅 2026-05-19


### Auditing Agent Harness Safety
- **英文标题**: Auditing Agent Harness Safety
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Chengzhi Liu, Yichen Guo, Yepeng Liu
- **论文链接**: https://huggingface.co/papers/2605.14271
---

### MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning
- **英文标题**: MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Yaolun Zhang, Yujie Zhao, Nan Wang
- **论文链接**: https://huggingface.co/papers/2605.14212
---

### CM-EVS: Sparse Panoramic RGB-D-Pose Data for Complete Scene Coverage
- **英文标题**: CM-EVS: Sparse Panoramic RGB-D-Pose Data for Complete Scene Coverage
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Jiale Liu, Jungang Li, Jieming Yu
- **论文链接**: https://huggingface.co/papers/2605.15597
---

### PhysBrain 1.0 Technical Report
- **英文标题**: PhysBrain 1.0 Technical Report
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Shijie Lian, Bin Yu, Xiaopeng Lin
- **论文链接**: https://huggingface.co/papers/2605.15298
---

### From Plans to Pixels: Learning to Plan and Orchestrate for Open-Ended Image Editing
- **英文标题**: From Plans to Pixels: Learning to Plan and Orchestrate for Open-Ended Image Editing
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Anirudh Sundara Rajan, Krishna Kumar Singh, Yong Jae Lee
- **论文链接**: https://huggingface.co/papers/2605.15181
---

### Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards
- **英文标题**: Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Mengjie Ren, Jie Lou, Boxi Cao
- **论文链接**: https://huggingface.co/papers/2605.14539
---

### Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR
- **英文标题**: Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Chanuk Lee, Sangwoo Park, Minki Kang
- **论文链接**: https://huggingface.co/papers/2605.15726
---

### MMSkills: Towards Multimodal Skills for General Visual Agents
- **英文标题**: MMSkills: Towards Multimodal Skills for General Visual Agents
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Kangning Zhang, Shuai Shao, Qingyao Li
- **论文链接**: https://huggingface.co/papers/2605.13527
---

### FFAvatar: Few-Shot, Feed-Forward, and Generalizable Avatar Reconstruction
- **英文标题**: FFAvatar: Few-Shot, Feed-Forward, and Generalizable Avatar Reconstruction
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Thuan Hoang Nguyen, Jiahao Luo, Yinyu Nie
- **论文链接**: https://huggingface.co/papers/2605.15320
---

### Flash-GRPO: Efficient Alignment for Video Diffusion via One-Step Policy Optimization
- **英文标题**: Flash-GRPO: Efficient Alignment for Video Diffusion via One-Step Policy Optimization
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Xiaoxuan He, Siming Fu, Zeyue Xue
- **论文链接**: https://huggingface.co/papers/2605.15980
---

### HodgeCover: Higher-Order Topological Coverage Drives Compression of Sparse Mixture-of-Experts
- **英文标题**: HodgeCover: Higher-Order Topological Coverage Drives Compression of Sparse Mixture-of-Experts
- **来源**: 🤗 HuggingFace  ⭐ 5/10
- **作者**: Tao Zhong, Dongzhe Zheng, Christine Allen-Blanchette
- **论文链接**: https://huggingface.co/papers/2605.13997
---
## 📅 2026-05-18

今日无新增 OR 相关研究
## 📅 2026-05-17

今日无新增 OR 相关研究
## 📅 2026-05-17

今日无新增 OR 相关研究
## 📅 2026-05-17

今日无新增 OR 相关研究
## 📅 2026-05-17

今日无新增 OR 相关研究
## 📅 2026-05-16

根据强制筛选原则，对提供的论文列表进行分析后，筛选出属于“AI + 运筹优化”交叉领域的高质量研究如下：

---
### 大规模多智能体路径规划中的局部通信学习
- **英文标题**: Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding
- **作者**: 未在摘要中明确列出主要作者
- **核心贡献**: 提出了一种基于机器学习的、可扩展的、去中心化的多智能体路径规划求解器，通过局部通信解决大规模MAPF问题。
- **实践价值**: 可应用于物流仓储中的多机器人协同、搜救任务中的多智能体协调等大规模、实时性要求高的场景。
- **OR 技术关键词**: 多智能体路径规划 (MAPF), 强化学习, 路径规划 (PATHFINDING), 调度 (SCHEDULING)
- **论文链接**: https://huggingface.co/papers/2605.07637
---
### 超越单一智能：基于LLM的多智能体系统中的协作、失败归因与自我进化研究综述
- **英文标题**: Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems
- **作者**: 未在摘要中明确列出主要作者
- **核心贡献**: 系统性综述了基于大语言模型的多智能体系统中的协作机制、故障诊断与传播，以及系统自我进化的挑战与方法。
- **实践价值**: 为设计更可靠、更具鲁棒性的复杂自动化系统和决策支持系统提供理论框架，例如在供应链协调、智能制造产线管理等场景下，多智能体系统的鲁棒性至关重要。
- **OR 技术关键词**: 多智能体系统, 鲁棒性 (Robustness), 决策支持 (Decision Support)
- **论文链接**: https://huggingface.co/papers/2605.14892
---
### SPIN：面向工业任务的结构化LLM规划与迭代导航
- **英文标题**: SPIN: Structural LLM Planning via Iterative Navigation for Industrial Tasks
- **作者**: 未在摘要中明确列出主要作者
- **核心贡献**: 提出SPIN框架，一种结合了有向无环图（DAG）验证和前缀式执行控制的规划方法，确保LLM生成的工业工作流程结构有效且长度最优。
- **实践价值**: 适用于工业自动化场景中的任务规划与执行，例如生产调度（PRODUCTION SCHEDULING）、物流流程编排等，通过减少无效规划和API调用成本来提升效率。
- **OR 技术关键词**: 规划 (PLANNING), 调度 (SCHEDULING), 生产 (PRODUCTION), 图优化 (DAG)
- **论文链接**: https://huggingface.co/papers/2605.14051
---
### Nexus：面向时间序列预测的智能体框架
- **英文标题**: Nexus: An Agentic Framework for Time Series Forecasting
- **作者**: 未在摘要中明确列出主要作者
- **核心贡献**: 提出了一个多智能体框架Nexus，通过融合时间序列基础模型（数值模式专家）和LLM（非结构化文本推理专家），解决了跨领域时间序列预测对上下文信号的需求。
- **实践价值**: 可直接应用于销售预测（DEMAND FORECAST）、库存管理（INVENTORY MANAGEMENT）、供应链风险预测（SUPPLY CHAIN RISK FORECAST）等需要结合数值和事件信息的决策场景。
- **OR 技术关键词**: 预测 (FORECAST), 供应链 (SUPPLY CHAIN), 库存 (INVENTORY), 决策制定 (DECISION MAKING), 多智能体系统
- **论文链接**: https://huggingface.co/papers/2605.14389
---
### 动态潜路由
- **英文标题**: Dynamic Latent Routing
- **作者**: 未在摘要中明确列出主要作者
- **核心贡献**: 提出了广义Dijkstra搜索（GDS）算法，并基于其原理设计了动态潜路由（DLR）方法，将马尔可夫决策过程（MDP）中的最优策略分解并组合为子策略，同时学习离散潜代码以实现高效路由。
- **实践价值**: 可应用于需要战略规划与路径选择的复杂决策问题，例如物流网络中的动态路由（ROUTE OPTIMIZATION）、调度（SCHEDULING）问题的求解。
- **OR 技术关键词**: 路径规划 (ROUTE), 调度 (SCHEDULING), 马尔可夫决策过程 (MDP), 决策制定 (DECISION MAKING)
- **论文链接**: https://huggingface.co/papers/2605.14323
---